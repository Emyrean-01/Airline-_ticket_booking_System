...
import in.co.airlineticket.dao.AirportDAOInt;
...
public class AirportServiceImpl implements AirportServiceInt {
	...
	public long add(AirportDTO dto) throws DuplicateRecordException {
		...
		AirportDTO code = dao.findByCode(dto.getCode());
		if(code !=null) 
			throw new DuplicateRecordException("Airport Code already exist");
		{
		AirportDTO existdto = dao.findByName(dto.getName());
		if (existdto != null)
			throw new DuplicateRecordException("Airport with this Name already exist"); 
		}
		long pk = dao.add(dto);
	...
	public AirportDTO findByCode(String code) {
		log.info("AirportServiceImpl findByAirportCode method start");
		AirportDTO dto = dao.findByCode(code);
		log.info("AirportServiceImpl findByAirportCodethod end");
		return dto;
	}
	...
	public void update(AirportDTO dto) throws DuplicateRecordException {
		...
		if (existdto != null && dto.getId() != existdto.getId())
			throw new DuplicateRecordException("Details already exist");
		dao.update(dto);
