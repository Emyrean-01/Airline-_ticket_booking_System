...
import in.co.airlineticket.form.AirportForm;
import in.co.airlineticket.service.AirportServiceInt;
	...
	public String submit(@RequestParam String operation, @Valid @ModelAttribute("form") AirportForm form,
		...
		try {
			if (OP_SAVE.equalsIgnoreCase(form.getOperation())) {
				...
				if (dto.getId() > 0) {
					service.update(dto);
					model.addAttribute("success", "Airport Details Updated Successfully");
				} else {
					service.add(dto);
					model.addAttribute("success", "Airport Added Successfully");
				}
				return "airport";
			}
	...
	public String searchList(HttpSession session, @ModelAttribute("form") AirportForm form,
		...
		if (OP_DELETE.equalsIgnoreCase(operation)) {
			...
			if (form.getIds() != null) {
				...
				model.addAttribute("success", "Airport Details has been removed successfully");
			} else {
		...
		List<AirportDTO> list = service.search(dto, pageNo, pageSize); //Search From First Page

		List<AirportDTO> totallist = service.search(dto); //Search From Complete List
