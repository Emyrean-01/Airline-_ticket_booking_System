...
import in.co.airlineticket.form.FlightForm;
	...
	public String submit(@RequestParam String operation, @Valid @ModelAttribute("form") FlightForm form,
		...
		try {
			if (OP_SAVE.equalsIgnoreCase(form.getOperation())) {
				...
				if (dto.getId() > 0) {
					service.update(dto);
					model.addAttribute("success", "Flight Details Updated Successfully");
				} else {
					service.add(dto);
	...
	public String searchList(HttpSession session, @ModelAttribute("form") FlightForm form,
		...
		if (OP_DELETE.equalsIgnoreCase(operation)) {
			...
			if (form.getIds() != null) {
				...
				model.addAttribute("success", "Flight Details has been removed successfully");
			} else {
