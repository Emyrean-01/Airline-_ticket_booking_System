...
import in.co.airlineticket.dto.InvoiceDTO;
...
import in.co.airlineticket.service.InvoiceServiceInt;
...
public class InvoiceCtl extends BaseCtl {
	...
	InvoiceForm fm = new InvoiceForm();
	...
	public String searchList(HttpSession session,@ModelAttribute("form") InvoiceForm form,
		...
		//if (fm.getDepartureDate()< now.now())
		if (OP_CANCEL.equalsIgnoreCase(operation)) {
			if (form.getIds() != null) {
				...
				model.addAttribute("success", "Booking has been Canceled and refund request is initiated.");
			} else {
